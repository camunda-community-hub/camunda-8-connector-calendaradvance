...
@OutboundConnector(name = "CalendarAdvanceFunction", inputVariables = {
...
public class CalendarAdvanceFunction implements OutboundConnectorFunction, CherryConnector {
    ...
    public static final List<Class<?>> allFunctions = Arrays.asList(DayFunction.class,
            HourFunction.class);
    ...
    public String getDescription() {
        return "Advance a date in calendar in time or day, using duration and business calendar, holidays: " + getListSubFunctions().stream()
                .map(SubFunction::getSubFunctionDescription)
    ...
    public Map<String, String> getListBpmnErrors() {
        ...
        allErrors.put(CalendarAdvanceError.ERROR_UNKNOWN_FUNCTION, CalendarAdvanceError.ERROR_UNKNOWN_FUNCTION_EXPLANATION);
        allErrors.put(CalendarAdvanceError.ERROR_BAD_INPUTPARAMETER, CalendarAdvanceError.ERROR_BAD_INPUTPARAMETER_EXPLANATION);

        for (SubFunction subFunction : getListSubFunctions()) {
    ...
    public Class<?> getInputParameterClass() {
        return CalendarAdvanceInput.class;
    }
     ...
     */
    public List<SubFunction> getListSubFunctions() {
        List<SubFunction> listSubFunction = new ArrayList<>();

        for (Class<?> classFunction : allFunctions) {
            try {
                Constructor<?> constructor = classFunction.getConstructor();
                SubFunction inputSubFunction = (SubFunction) constructor.newInstance();
